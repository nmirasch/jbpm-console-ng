...
public class DataSetHandlerImpl implements DataSetHandler {
    ...
    public boolean filter( DataSetGroup op ) {
        ...
        if ( cg == null )
            throw new RuntimeException( FiltersConstants.INSTANCE.datasethandler_groupops_no_pivotcolumn() );
        if ( !op.isSelect() )
            throw new RuntimeException( FiltersConstants.INSTANCE.datasethandler_groupops_no_groupintervals() );
